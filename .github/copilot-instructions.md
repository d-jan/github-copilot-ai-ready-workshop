...
Git / GitHub の運用ルール（ブランチ・Issue・コミット）は
`.github/git-github-workflow.md` に分離している。詳細はそちらに従うこと。

- **セッション開始時（最初にファイル変更・コミットを行う前）に一度だけ、
  同ドキュメントの「ブランチ運用ルール」に従ってブランチ名を規約に合わせること。**
  `ask_user` でリネーム要否とパターン（新機能／バグ修正）を確認する。
- Issue 対応・コミットの手順も同ドキュメントを参照する。
